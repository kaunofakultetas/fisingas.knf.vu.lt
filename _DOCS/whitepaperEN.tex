...
through the ORM --- the only raw SQL in the codebase is the migration
that converted the timestamp columns to native datetimes
(Section~\ref{sec:datamodel}). Every request runs in a transaction (\code{ATOMIC\_REQUESTS = True}), so a
...
(Table~\ref{tab:tables}). Every timestamp is a native
\code{timestamptz} column holding UTC; the API publishes it as
ISO-8601 in local time with the explicit offset
(\code{2026-08-26T19:09:07+03:00}, \code{+02:00} in winter) and
\code{null} for ``never''. All comparisons --- the dashboard's
``active in the last 30 minutes'' window, the retention sweep, the
frontend's date filters --- are made on real datetimes, so the
daylight-saving transitions are harmless and the display is correct in
...
\code{(student, question\_id, option\_id)} on options --- guarantee
that a student can never hold two snapshots of one question.
Concurrency itself is handled one level up: every request on the test
endpoints first updates the student's ``last seen'' timestamp, which
takes that student's row lock in PostgreSQL, so two simultaneous first
requests are serialised --- the second waits, then sees the first one's
rows and skips dealing. Should the constraint fire regardless, the
request logs it and serves the snapshot that landed; any other
integrity failure is logged and re-raised rather than answered with an
empty test. Combined with per-request transactions, a partially dealt
test cannot exist.

When the student finishes, a third freeze happens: the computed totals
are written once into \code{TestResult}, under the same row lock, so
an autosave that is still committing is included in the frozen totals
--- the grade the lists show and the per-question review can never
disagree. From then on every list view (admin table, public
leaderboard) serves that student from a single stored row instead of
re-judging their answers, so the hot pages do constant work per
finished student and live judging is reserved for students still
answering.
...
8-character minimum and a 72-byte maximum --- bcrypt's input limit; a
longer password would silently authenticate on its truncated prefix ---
enforced when accounts are created or updated, and an email address
must be unique on edit as well as on create.
...
login name --- in a new session row and sets an \code{HttpOnly} cookie
named \code{session}, configured to expire when the browser closes.
Logging out is server-side: \code{POST /api/logout} deletes the session
row and clears the cookie, so a cookie value captured earlier (shared
or kiosk machines) is dead from that moment; the login page issues that
call whenever it is opened, which is how the ``Atsijungti'' link works.
On every request the session name is re-resolved against the account
tables, and disabled accounts are treated as non-existent --- a
deactivated administrator or student loses any live session on their
next request.
...
per-view. Three access tiers result: public endpoints (login, logout,
...
\begin{enumerate}
  ...
  \item \textbf{Deal.} The first
        ...
        return the already-dealt set in the same order: the order is
        random per student and fixed at deal time, so the numbering
        in the navigation panel survives a reload. The pool is read
        once and the sample sized from that read, and a corrupt
        setting value falls back to the default like a missing one,
        so a first request cannot fail on a concurrent bank edit or a
        hand-edited setting.
  \item \textbf{Answering.} The frontend posts the \emph{entire}
        ...
        payload cannot write into another student's rows. The whole
        payload is validated before anything is written, so a
        malformed save is refused as a unit and never leaves a
        half-saved state; the frontend sends saves one at a time,
        coalescing clicks made while a save is in flight into a single
        follow-up, so an older state can never overwrite a newer one.
        Saving is idempotent and repeatable; a student can close the
        browser and resume with their name and code.
  \item \textbf{Finish.} \code{GET /api/student/finish} takes the
        student's row lock, grades the frozen rows, writes the
        \code{TestResult} totals and sets the finished flag; a
        student who is already finished is left untouched, so a
        double click is harmless. The frontend waits for any pending
        autosave to be acknowledged before it navigates to this call,
        so the last click is always part of the frozen grade. The
        step is irreversible: from now
...
\begin{tikzpicture}[
  ...
  \draw[msg] (6.6,-5.5) -- node[mlbl]{dealt questions (deal order)} (0,-5.5);
  ...
  \draw[msg] (6.6,-9.1) -- node[mlbl]{lock student row, grade snapshots, INSERT TestResult, set finished} (13.2,-9.1);
...
\begin{cases}
  0 & \text{if } a_q \neq v_q,\\[2pt]
  \max\bigl(0,\; 1 - 0.1\,n_q + 0.1\,c_q\bigr) & \text{if } a_q = v_q.
\end{cases}
\end{equation*}

That is, a correct verdict with every option right scores exactly 1;
each missed option costs 0.1, down to a floor of zero (so a question
with more than ten options can never subtract from the rest of the
test). If the main verdict is wrong the question scores 0 regardless
of the options. For a test of $N$ dealt
...
truncated integer percentage. The option totals are counted over the
correctly identified questions only --- exactly the options the formula
scores --- so a student who answered nothing shows $0/0$ rather than
credit for untouched checkboxes that happened to expect ``unchecked''.
$N$ counts \emph{dealt} questions, so unanswered questions count
against the grade exactly like wrong verdicts. A newly added option
expects ``not checked'' until the administrator says otherwise; an
expected value that was never set at all (possible only on rows older
than that rule) can never be matched.
...
server (one save in flight at a time, later clicks coalesced) and a
toast reports any failure, so losing a connection or closing the laptop
costs nothing: logging in again resumes exactly where the student left
off. A progress panel shows an answered counter, a progress bar, and a
jump grid with one button per question (filled when answered, ringed
when current). The finish button is a 1.5-second hold that waits for
the last autosave to be confirmed before leaving the page --- if it
cannot be delivered, the student is told and stays on the test rather
than finishing without those answers. The results page then shows the
grade headline,
...
grade and timestamps, sorted as numbers and dates rather than text. The detail page shows the credential card ---
...
insert and an empty password on edit means ``keep the current one''.
Passwords must be 8 characters to 72 bytes, email addresses unique,
and an administrator can neither disable nor delete their own account
--- so at least one working administrator always survives.
...
Every request is logged by Gunicorn to the container's standard output
(\code{docker logs fisingas-django}): the client address as forwarded
by Caddy, the request line, status, response size and duration.
...
night at 05:00 (an hour that exists on every day of the year; 03:00 is
the one daylight-saving time skips in spring and repeats in autumn) it
executes the \code{delete\_old\_students} management command inside
the Django container. The command deletes student
...
\code{django/runTests.sh} builds the backend image and runs the
regression suite --- over three hundred tests --- in a throwaway
container against an in-memory SQLite database, so no running stack is
needed. The suite covers every endpoint, the grading arithmetic, the
snapshot and freeze discipline, the retention sweep and the timestamp
policy; a contract layer validates live responses against
\code{swagger/swagger.yaml}, so the API documentation cannot drift from
the code unnoticed. Known, not-yet-fixed defects are pinned as
\emph{expected failures}: each has a test asserting the correct
behaviour, so the suite stays green while the defect stays documented
and executable, and a fix flips the test instead of passing silently.
...
\begin{itemize}
  ...
  \item \textbf{No audit trail.} Administrator actions and logins are
        not recorded beyond Gunicorn's request log, and there is no
        undo for a deleted student or question. Images live as
        database blobs, which is simple and consistent but makes the
        database the size bottleneck.
  \item \textbf{No time limit and no start timestamp.} Nothing records
        when a test was dealt, so the duration of a sitting can
        neither be enforced nor reconstructed afterwards; only the
        finish time is stored.
