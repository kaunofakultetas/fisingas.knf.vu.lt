...
bazės eina per ORM (kodo bazėje nėra nė vienos „žalios`` SQL
užklausos). Kiekviena užklausa vykdoma transakcijoje
...
(\ref{tab:tables}~lentelė). Sąmoninga keistenybė: nėra nė vieno
gimtojo datos/laiko stulpelio — kiekvienas laiko žymuo yra tekstinis
stulpelis su \code{YYYY-MM-DD HH:MM:SS} vietos laiku
(\code{Europe/Vilnius}). Kadangi formatas rikiuojasi leksikografiškai,
kodas laiko žymenis lygina kaip eilutes (tuo remiasi ir skydelio
„aktyvūs per paskutines 30 minučių`` langas, ir saugojimo terminų
valymas).
...
ir \code{(student, question\_id, option\_id)} variantams — kartu yra
ir lygiagretumo saugiklis: jei dvi vienalaikės pirmos užklausos abi
bando išdalyti, nusileisti gali tik vienas masinis įrašymas;
pralaimėjusiosios \code{IntegrityError} nuryjama ir ji tiesiog
perskaito laimėtojos kopiją. Kartu su transakcijomis kiekvienai
užklausai iš dalies išdalytas testas negali egzistuoti.

Studentui baigus įvyksta trečias užšaldymas: suskaičiuotos sumos
vieną kartą įrašomos į \code{TestResult}. Nuo tada kiekvienas sąrašo
vaizdas (administratoriaus lentelė, vieša lyderių lentelė) šį studentą
...
keičiant paskyrą reikalaujama bent 8 simbolių. Studentų prisijungimo
...
išsaugo vieną reikšmę — prisijungimo vardą — ir nustato slapuką vardu
\code{session}, galiojantį iki naršyklės uždarymo. Slapukas sąmoningai
\emph{ne} \code{HttpOnly}: serveryje nėra atsijungimo galinio taško,
o prisijungimo puslapis naudotoją atjungia ištrindamas slapuką iš
JavaScript. Kiekvienoje užklausoje sesijos vardas iš naujo
...
(prisijungimas, registracija, lyderių lentelė, skaidrės),
...
\begin{enumerate}
  ...
  \item \textbf{Išdalijimas.} Pirmasis
        ...
        Vėlesni kvietimai grąžina jau išdalytą rinkinį. Serveris
        klausimų tvarką permaišo kiekviename GET; sąsaja sesijos metu
        laikosi savo pastovios tvarkos.
  \item \textbf{Atsakinėjimas.} Sąsaja po kiekvieno paspaudimo
        ...
        krovinys negali rašyti į kito studento eilutes. Saugojimas
        idempotentinis ir kartotinas; studentas gali užverti naršyklę
        ir tęsti prisijungęs su vardu ir kodu.
  \item \textbf{Baigimas.} \code{GET /api/student/finish} įvertina
        užšaldytas eilutes, įrašo \code{TestResult} sumas
        (\code{update\_or\_create}, tad dvigubas paspaudimas
        nekenksmingas) ir nustato baigimo žymę. Žingsnis
        negrįžtamas: nuo šiol klausimų galinis taškas šiam studentui
...
\begin{tikzpicture}[
  ...
  \draw[msg] (6.6,-5.5) -- node[mlbl]{išdalyti klausimai (permaišyti)} (0,-5.5);
  ...
  \draw[msg] (6.6,-9.1) -- node[mlbl]{įvertinti kopijas, INSERT TestResult, pažymėti baigtą} (13.2,-9.1);
...
\begin{cases}
  0 & \text{jei } a_q \neq v_q,\\[2pt]
  1 - 0{,}1\,n_q + 0{,}1\,c_q & \text{jei } a_q = v_q.
\end{cases}
\end{equation*}

Tai yra, teisingas verdiktas su visais teisingais variantais duoda
lygiai 1; kiekvienas praleistas variantas kainuoja 0,1. Jei
pagrindinis verdiktas klaidingas, klausimas vertinamas 0
nepriklausomai nuo variantų. Testui iš $N$ išdalytų klausimų
...
teisingų dalį nukirsto sveiko skaičiaus procentais. $N$ skaičiuoja
\emph{išdalytus} klausimus, tad neatsakyti klausimai į įvertinimą
įeina lygiai kaip klaidingi verdiktai.
...
išsaugo visą atsakymų būseną serveryje, o nesėkmę praneša
...
palaikymas; rezultatų puslapis tada rodo įvertinimo antraštę, tris
...
žymomis. Kortelės puslapis rodo prisijungimo duomenų kortelę —
...
„palikti dabartinį``, minimalus ilgis — 8 simboliai.
...
su į atvaizdą įkepta crontab — vykdo vieną darbą: kas naktį 03:00
Django konteineryje paleidžia valdymo komandą
\code{delete\_old\_students}. Komanda ištrina studentų paskyras,
...
\begin{itemize}
  ...
  \item \textbf{Nėra serverio pusės atsijungimo.} Sesijos baigiasi
        užvėrus naršyklę arba ištrynus slapuką kliento pusėje;
        pavogtas slapukas iki tol lieka galiojantis.
  \item \textbf{Eksploataciniai saugikliai ploni.} Administratorius
        gali ištrinti savo ar paskutinę administratoriaus paskyrą;
        testo dydis API lygmenyje priima bet kokį sveikąjį skaičių;
        paveikslėliai gyvena duomenų bazės blokais — paprasta ir
        nuoseklu, bet duomenų bazė tampa dydžio butelio kakleliu.
